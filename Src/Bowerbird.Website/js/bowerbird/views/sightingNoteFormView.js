...
function ($, _, Backbone, app, ich, SightingDetailsView, IdentificationFormView, SightingNoteSubFormView, sightingNoteDescriptions, moment) {
    ...
    var SightingNoteFormView = Backbone.Marionette.Layout.extend({
        ...
        onValidation: function (obs, errors) {
            ...
            this.$el.find('textarea.note-description, .tagit').removeClass('input-validation-error');
         
            if (_.any(errors, function (item) { return item.Field === 'Descriptions'; })) {
                this.$el.find('textarea.note-description, .tagit').addClass('input-validation-error');
            }
        },
